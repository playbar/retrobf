...
static void mui_context_reset_textures(mui_handle_t *mui)
{
    ...
    iconpath[0] = '\0';
    fill_pathname_application_special(iconpath, sizeof(iconpath), APPLICATION_SPECIAL_DIRECTORY_ASSETS_MATERIALUI_ICONS);
                                ...
                                const char *grid[], unsigned id)
{
    ...
    menu_display_draw_quad(0, height/2.0, width, height/2.0, width, height, &dark[0]);
                               ...
                               float *tabs_bg_color, float *tabs_separator_color)
{
    ...
    /* tabs background */
    menu_display_draw_quad(0, height - mui->tabs_height, width, mui->tabs_height, width, height, tabs_bg_color);
    /* tabs separator */
    menu_display_draw_quad(0, height - mui->tabs_height, width, 1, width, height, tabs_separator_color);
}
...
static float mui_content_height(void)
{
    ...
    {
        mui_node_t *node  = (mui_node_t*) menu_entries_get_userdata_at_offset(list, i);
        sum += node->line_height;
    }
    return sum;
}

static void mui_draw_scrollbar(mui_handle_t *mui, unsigned width, unsigned height, float *coord_color)
{
                                  ...
                                  const char *message, float *body_bg_color, uint32_t font_color)
{
    ...
    if (menu_input_dialog_get_display_kb())
        mui_render_keyboard(mui, video_info, menu_event_get_osk_grid(), menu_event_get_osk_ptr());
...
static void mui_render(void *data, bool is_idle)
{
    ...
    if (settings->bools.menu_pointer_enable)
    {
        ...
        {
            mui_node_t *node = (mui_node_t*) menu_entries_get_userdata_at_offset(list, ii);

            if (pointer_y > (-mui->scroll_y + header_height + node->y) && pointer_y < (-mui->scroll_y + header_height + node->y + node->line_height))
    ...
    if (settings->bools.menu_mouse_enable)
    {
        ...
        for (ii = 0; ii < menu_entries_get_size(); ii++)
        {
            ...
            if (mouse_y > (-mui->scroll_y + header_height + node->y) && mouse_y < (-mui->scroll_y + header_height + node->y + node->line_height))
    ...
    if (mui_content_height() < height - header_height - mui->tabs_height)
        mui->scroll_y = 0;
        ...
        float *menu_list_color)
{
    ...
    size_t len = menu_entries_get_end();
...
static void mui_frame(void *data, video_frame_info_t *video_info)
{
    ...
    if (node)
        menu_display_draw_quad(0, header_height - mui->scroll_y + node->y, width, node->line_height, width, height, &blue_50[0]);
    ...
    /* header */
    menu_display_draw_quad(0, 0, width, header_height, width, height, &header_bg_color_real[0]);
    ...
    menu_display_draw_quad(0, header_height, width, mui->shadow_height, width, height, &shadow_bg[0]);
    title_margin = mui->margin;
    if (menu_entries_ctl(MENU_ENTRIES_CTL_SHOW_BACK, NULL))
    {
        title_margin = mui->icon_size;
        mui_draw_icon(mui->icon_size, mui->textures.list[MUI_TEXTURE_BACK], 0, 0, width, height, 0, 1, &pure_white[0]);
    }
    ...
    menu_display_draw_text(mui->font, title_buf, title_margin, header_height / 2 + mui->font->size / 3,
    ...
    if (menu_input_dialog_get_display_kb())
    {
        ...
        if (body_bg_color_ptr)
            mui_render_messagebox(mui, video_info, msg, body_bg_color_ptr, font_hover_color);
    }
    ...
    if (!string_is_empty(mui->box_message))
    {
        ...
        if (body_bg_color_ptr)
            mui_render_messagebox(mui, video_info, mui->box_message, body_bg_color_ptr, font_hover_color);
        mui->box_message[0] = '\0';
    }

    if (mui->mouse_show)
        menu_display_draw_cursor(&white_bg[0], mui->cursor.size, mui->textures.list[MUI_TEXTURE_POINTER],
                                 menu_input_mouse_state(MENU_MOUSE_X_AXIS), menu_input_mouse_state(MENU_MOUSE_Y_AXIS),
                                 width, height);
